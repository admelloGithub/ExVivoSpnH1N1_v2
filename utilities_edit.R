...
{
    getOption("KEGG_GENOME_URL", "https://rest.genome.jp")
}
...
keggLink <- function(target, source)
{
  if (missing(source))
  {
    url <- sprintf("%s/link/%s",
                   .getGenomeUrl(), target)
    ass <- .getUrl(url, .matrixParser, ncol=3)
  } else {
    url <- sprintf("%s/link/%s/%s",
                   .getRootUrl(), target, paste(source, collapse="+"))
   ass <- .getUrl(url, .listParser, nameColumn=1, valueColumn=2)
...
.getUrl <- function(url, parser, ...)
{
  ...
  if (debug)
    .printf("url == %s", url)
  response <- GET(gsub("http","https",url))
...
download_KEGGfile<-function(pathway_id="00010",species='hsa',target_dir=getwd()) {
  ...
  downloadKeggFile<-function(pathway_id="hsa00010",species='hsa',target_dir=getwd()) {
    ...
    httpHeaderRefer=paste0("https://www.kegg.jp/kegg-bin/show_pathway?org_name=",species)
